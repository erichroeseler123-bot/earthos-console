...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    return res.status(200).json({ ok: true, service: 'redrocksdd-email', address: SUPPORT_ADDRESS, forwardsTo: FORWARD_TO });
  }
  ...
  try {
    ...
    return res.status(200).json({ ok: true, forwarded: true, id: forwarded?.id || null });
  } catch (error) {
